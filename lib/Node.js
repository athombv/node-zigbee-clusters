...
class Node extends EventEmitter {
    ...
    _makeErrorResponse(endpointId, clusterId, frame) {
        ...
        result.frameControl = frame.frameControl;
    ...
    async handleFrame(endpointId, clusterId, frame, meta) {
        ...
        try {
            if (this.endpoints[endpointId]) {
                ...
                if(response && (result instanceof ZCLStandardHeader || result instanceof ZCLMfgSpecificHeader)) {
                    response = result;
                }
            } else {
        ...
        if(response) {
            this.sendFrame(endpointId, clusterId, response);
        }
    }
}
