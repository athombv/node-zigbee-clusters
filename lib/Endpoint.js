...
class Endpoint extends EventEmitter {
   ...
   * @param {number[]} descriptor.outputClusters
   */
  ...
  /**
   * Checks if `clusterName` is an `inputCluster` on this endpoint.
   * @param {string} clusterName - Cluster#NAME
   * @returns {boolean}
   */
  hasInputCluster(clusterName) {
    if (typeof clusterName !== 'string') throw new TypeError('Error: `clusterName` must be string');
    const cluster = Cluster.getCluster(clusterName) || {};
    const clusterId = cluster.ID;
    return (this._descriptor.inputClusters || []).includes(clusterId);
  }

  /**
   * Checks if `clusterName` is an `outputCluster` on this endpoint.
   * @param {string} clusterName - Cluster#NAME
   * @returns {boolean}
   */
  hasOutputCluster(clusterName) {
    if (typeof clusterName !== 'string') throw new TypeError('Error: `clusterName` must be string');
    const cluster = Cluster.getCluster(clusterName) || {};
    const clusterId = cluster.ID;
    return (this._descriptor.outputClusters || []).includes(clusterId);
  }
