...
class Cluster extends EventEmitter {
    constructor(endpoint, binding) {
        ...
        //Ids are not unique
        this._commandsById = Object.entries(this._commands).reduce((r, [name, _cmd]) => {
            const cmd = {..._cmd, name};
            if(cmd.args) {
                cmd.args = ZCLStruct(this.name+'.'+name, cmd.args);
                if(_cmd === GLOBAL_COMMANDS.defaultResponse) {
                    this._defaultResponseArgsType = cmd.args;
                }
            }
            if(r[cmd.id]) {
                r[cmd.id].push(cmd);
            ...
            if(cmd.response) {
                const res = {...cmd.response, name: name+'.response', response: true};
                if(typeof res.id !== 'number') {
                    res.id = cmd.id;
                }
                if(res.args) {
                    res.args = ZCLStruct(this.name+'.'+res.name, res.args);
                }
        ...
        for(const cmdName in commands) {
            Object.defineProperty(proto, cmdName, {
                value: {[cmdName]: async function(args) {
                    ...
                    if(!cmd.response && response instanceof this._defaultResponseArgsType && response.status !== 'SUCCESS') {
