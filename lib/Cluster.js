        ...
        if(command) {
            ...
            if(handler) {
                ...
                if(command.response && command.response.args && !frame.frameControl.disableDefaultResponse) {
                    return new command.response.args(response);
                }
                //TODO: response without args, return ZCLFrame
                return;
            }
        }
        ...
        for(const cmdName in commands) {
            Object.defineProperty(proto, cmdName, {
                value: {[cmdName]: async function(args) {
                    ...
                    if(!cmd.response && response instanceof this.constructor.defaultResponseArgsType) {
