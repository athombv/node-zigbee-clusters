...
class Cluster extends EventEmitter {
  ...
  async readAttributes(attributes, opts) {
    ...
    for (const attr of attributes) {
      if (typeof attr === 'number') {
        ...
        numericIds.add(attr);
        if (!idToName[attr]) idToName[attr] = attr;
        attrIds.add(attr);
  ...
  async writeAttributes(attributes = {}, opts) {
    ...
    const response = await super.writeAttributes({ attributes: data, manufacturerId }, opts);

    // Check for write errors in the response
    if (response && Array.isArray(response.attributes)) {
      const errors = response.attributes.filter(a => a.status !== 'SUCCESS');
      if (errors.length > 0) {
        const details = errors.map(e => `${e.id}: ${e.status}`).join(', ');
        throw new Error(`Write failed: ${details}`);
      }
    }
