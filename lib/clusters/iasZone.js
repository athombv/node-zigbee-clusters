...
class IASZoneCluster extends Cluster {
  ...
  /**
   * This command is generated on the cluster instead of the bound cluster
   * @param {zoneType: string, manufacturerCode: number} payload
   */
  onZoneEnrollRequest(payload) {
    this.emit('zoneEnrollRequest', payload);
  }
  ...
  onZoneStatusChangeNotification(payload) {
    this.emit('zoneStatusChangeNotification', payload);
  }
