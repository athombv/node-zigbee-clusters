...
declare module "zigbee-clusters" {
  ...
  namespace zclFrames {
    ...
    function ZCLConfigureReportingRecords({ withStatus }: {withStatus?: boolean}): ZCLDataType<{
      attributeId: number,
      status?: ZCLEnum8Status,
      ...
      minInterval: number,
      maxInterval: number,
      minChange: number, // TODO Check whether this is actually a number, or a buffer like the JS docs imply
    }>
  }
  ...
  abstract class Cluster<Attributes extends AttributeDefinitions, Commands extends CommandDefinitions> extends EventEmitter {
    ...
    readAttributes<K extends keyof Attributes | number>(attributes: ReadonlyArray<K>, opts?: {timeout?: number}): Promise<{[attribute in K]: AttributesFromDefinition<Attributes>[attribute]}>;
    writeAttributes<K extends keyof Attributes>(attributes: {[attribute in K]: AttributesFromDefinition<Attributes>[attribute]}, opts?: {timeout?: number}): Promise<{[attribute in K]: {id: number, status: 'SUCCESS' | 'FAILURE'}}>

    configureReporting(attributes: AttributeReportingConfiguration<Extract<keyof Attributes, string>>, opts?: {timeout?: number}): Promise<void>;
    readReportingConfiguration(attributes: ReadonlyArray<string | number>, opts?: {timeout?: number}): Promise<Array<ReadReportingConfigurationResponse>>;
    ...
    attributesById: {[Attribute in keyof Attributes as Attributes[Attribute]['id']]: { name: Attribute } & Attributes[Attribute] };

    commands: Commands;
    commandsById: {[Command in keyof Commands as Commands[Command]['id']]: Array<{name: Command} & Commands[Command]>};
  }
